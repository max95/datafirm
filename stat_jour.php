...
$result = mysqli_query($db,$select) or die ('Erreur : '.mysql_error() );
$total_pages_visitees_depuis_creation = mysqli_num_rows($result);
mysqli_free_result($result);

// on cherche le nombre de visiteurs depuis le début (création du site)
$sql = 'SELECT DISTINCT(ip) FROM statistiques';
$result = mysqli_query($db, $sql) or die('Erreur SQL !<br />'.$sql.'<br />'.mysql_error());
$total_visiteur_depuis_debut = mysqli_num_rows($result);
mysqli_free_result($result);
...
$result = mysqli_query($db, $sql) or die('Erreur SQL !<br />'.$sql.'<br />'.mysql_error());
$total_visiteur = mysqli_num_rows ($result);
mysqli_free_result($result);
